...
def main():
    ...
    if not listen and len(target) and port > 0:
        ...
        # データ送信
        client_sender(buffer.encode(encoding=enco))
...
def client_sender(buffer: bytes):
    ...
    try:
        ...
        while True:
            ...
            # データの送信
            client.send(buffer.encode(encoding=enco))
...
def client_handler(client_socket):
    ...
    if len(upload_destination):
        ...
        try:
            ...
            file_descriptor.write(file_buffer.encode(encoding=enco))
    ...
    if command:
        ...
        # プロンプトの表示
        prompt = "<BHP:#> "
        client_socket.send(prompt.encode(encoding=enco))
        ...
        while True:
            ...
            # コマンドの実行結果を送信
            client_socket.send(response.encode(encoding=enco))
